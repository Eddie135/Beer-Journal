...
- 已创建 Capacitor 8 Android 外壳并成功编译本机 debug APK；它通过 HTTPS 访问现有 Django 应用，不复制或改变业务逻辑。v4.0-F2.2 已更新版本号、沉浸式状态栏配置、返回键双击退出和键盘收起逻辑；真机安装与交互流程仍待验收。

## 下一步

下一步将最新 debug APK 安装到 Android 真机，验收启动、注册/登录、返回键、国家选择、照片上传和底部导航；确认后再单独创建 release 签名与 release APK/AAB。
