...
- 已创建 Capacitor 8 Android 工程并完成 F2.3 审计修正。产品路线现转为本地优先 v1.0：APK 将使用 `mobile/web/` 内置前端和本地 SQLite，不再通过 HTTPS 访问 Django；现有 Django/PostgreSQL/生产文件保留给 v1.1 同步后端。

## 下一步

当前正在执行本地优先 v1.0 的 L1：完成模型/页面审计、本地架构设计、本地前端骨架和离线启动 debug APK。L1 完成后停止，不提前进入 SQLite CRUD。
