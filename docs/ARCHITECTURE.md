...
PWA 并不要求使用单页应用。Django 页面同样可以提供 manifest、图标、Service Worker、响应式布局和“添加到主屏幕”。Android 当前使用 Capacitor 8 作为轻量原生 WebView 外壳，通过 HTTPS 加载既有站点；它不新增公开 API、不复制前端页面，也不改变 Django 的登录或数据边界。若未来需要离线同步或原生业务能力，再评估受控 API。
...
- Android 外壳使用 Capacitor 8、JDK 21、Android SDK Platform 36、Build-Tools 35.0.0 与工程自带 Gradle 8.14.3 Wrapper；debug APK 不可用于正式发布。
...
## 17. v1.0 本地优先架构（已确认，L1）

产品路线已调整为先交付完全本地运行的 Android APK。v1.0 的运行边界如下：

```text
Capacitor Android APK
  ├─ mobile/web/ 本地前端资源
  ├─ SQLite（后续 L2 实现）
  └─ Capacitor Filesystem（后续 L4 实现）
```

- Capacitor `webDir` 固定为 `mobile/web`，不再配置生产 `server.url` 或 `allowNavigation`。
- 本地前端使用原生 HTML、CSS 和 Vanilla JavaScript；L1 先提供主页、底部导航和二级页面骨架，L2-L4 再接入数据访问层、SQLite、照片和备份。
- v1.0 不运行 Django，不依赖 PostgreSQL、Nginx、HTTPS 或任何网络请求；现有 Django/PostgreSQL/生产部署文件继续保留，作为 v1.1 同步后端基础。
- Android 原生层继续保留沉浸式状态栏、底部安全区、系统返回键和生命周期恢复能力；本地前端负责路由状态和页面重绘。
- 不引入 Vue、React、FastAPI、Redis 或其他额外服务，避免为单用户离线应用增加构建和维护复杂度。

### L1 验收边界

- APK 在没有网络的情况下可以打开本地主页并显示空收藏状态；
