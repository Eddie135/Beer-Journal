...
生产环境只需要三个长期运行的服务：`web`、`db` 和 `caddy`。不加入 Redis、队列、独立前端服务器或对象存储。
...
- Caddy 使用受支持的 2.x 稳定版本，并持久化其证书数据。
...
本地第一阶段暂不需要 Caddy，浏览器通过本机端口访问；生产配置再启用 Caddy。

### 10.2 Debian 13 生产

- Docker Engine 和 Compose v2 插件；
- `web` 使用 Gunicorn，不使用 Django 开发服务器；
- `db`、照片和 Caddy 证书使用持久化卷；
- 数据库端口不暴露到互联网；
- 只有 Caddy 的 80/443 端口对外开放；
- 容器设置健康检查和重启策略；
