...
## RC2 Shell 内部回归（2026-07-18）

截图目录：`mobile/tests/android/.artifacts/shell-regression/`。

通过：Beer 列表、新建 Beer 顶部/底部、饮用记录列表、新建 Tasting 顶部/底部、个人数据、Filter Sheet、CountryPicker；层级中 Header、route-content、Bottom Navigation 均保持单实例，Filter Sheet 与 CountryPicker 顺序正常。

阻塞：内部包数据库为空，无法真实进入 Beer/Tasting 详情和编辑；新建 Beer 顶部静止画面仍能看到固定底栏覆盖中段字段，虽可继续滚动到保存按钮，需后续共同修复滚动/底栏布局。

日志：清空后重启，FATAL EXCEPTION、E/AndroidRuntime、SQLiteException、TAG_ID_INVALID、database is locked、console.error、unhandled rejection 均为 0。
